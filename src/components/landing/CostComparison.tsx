...
import { Check, X, AlertTriangle, ArrowRight, DollarSign } from 'lucide-react'
  ...
  return (
    <section id="comparison" className="py-20 md:py-28 bg-[#f6f4f0]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="bg-[#1a1918] text-[#f6f4f0] border-2 border-[#1a1918] rounded-sm p-7 space-y-6 flex flex-col justify-between shadow-[0_10px_35px_rgba(0,0,0,0.12)] relative">
            ...
            <div className="space-y-4">
              ...
              <ul className="space-y-3 pt-2 text-xs text-[#e8e4dc]">
                ...
                <li className="flex items-start gap-2.5">
                  <Check className="w-4 h-4 text-[#9e4733] shrink-0 mt-0.5" />
                  <span>Handles 10 calls at the exact same time without hold music.</span>
                </li>
                <li className="flex items-start gap-2.5">
                  <Check className="w-4 h-4 text-[#9e4733] shrink-0 mt-0.5" />
        ...
        {/* Real Example ROI Math Banner */}
        <div className="max-w-3xl mx-auto bg-[#ffffff] border border-[#e6e2d6] rounded-sm p-6 sm:p-7 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
          <div className="space-y-1">
            <div className="text-xs font-semibold uppercase tracking-wider text-[#9e4733]">
              Simple Return on Investment
            </div>
            <div className="font-serif text-lg font-bold text-[#1a1918]">
              If your average customer is worth $400...
            </div>
            <p className="text-xs text-[#66635e]">
              Capturing just 2 missed calls a week puts an extra <strong className="text-[#1a1918]">$3,200 in revenue</strong> directly into your business every month.
            </p>
          </div>

          <button
            onClick={onOpenDemo}
            className="bg-[#1a1918] hover:bg-[#2d2d2d] text-[#f6f4f0] text-xs font-semibold tracking-wider uppercase px-6 py-3 rounded-sm shrink-0 transition-all cursor-pointer"
          >
            Calculate Your ROI
          </button>
        </div>
